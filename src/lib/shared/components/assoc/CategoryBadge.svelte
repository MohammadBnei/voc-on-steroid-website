...
{#each wordCatList as wCat, i}
	...
	<input type="checkbox" id="{id + i}" class="modal-toggle" />
	<label for="{id + i}" class="modal modal-bottom sm:modal-middle">
		<div class="modal-box relative">
			<label for="{id + i}" class="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
			<h3 class="text-lg">Voulez-vous retirer la catégorie <span class="font-semibold">{wCat.name}</span> ?</h3>
			<div class="modal-action">
				<label for="{id + i}" class="btn btn-warning z-10" on:click="{() => unassignCategory(wordId, wCat)}"
					>Supprimer</label
				>
			</div>
		</div>
